...
while True:
    process_read = os.popen("ps -ef | grep crawling_coordinate.py | grep -v 'grep'").readlines()
    ...
    # 문자열로 변환한다.
    text_location=check_process.find("crawling_coordinate.py")
    # run24h.py가 몇번째 문자열인지 찾아낸다. 만약 문자열이 없으면, 즉 프로세스가 존재하지 않을 경우에는 -1을 반환한다.
    if ( text_location == -1 ):
        print("Process not found!")
        os.system("python crawling_coordinate.py &")
